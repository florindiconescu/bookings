...
func TestForm_Has(t *testing.T) {
	...
	has = form.Has("a")
	if !has {
		t.Error("Form shows does not have field when it has")
	}
}
...
func TestForm_MinLength(t *testing.T) {
	...
	if !form.Valid() {
		t.Error("form shows min length of 1 is not met it is")
	}
...
func TestForm_IsEmail(t *testing.T) {
	...
	postedValues = url.Values{}
	postedValues.Add("email", "me@testmail.co")
	form = New(postedValues)
	form.IsEmail("email")
	if !form.Valid() {
